...
def test_v2v_agent_session_streams_tokens_from_openai_client(monkeypatch):
    """V2VAgentSession.submit_user_turn yields delta.content tokens
    from the OpenAI-compatible streaming response."""
    from gateway.platforms.voice_rtc import V2VAgentSession
    ...
    monkeypatch.setattr(
        "gateway.platforms.voice_rtc.AsyncOpenAI",
        _FakeAsyncOpenAI,
    )
...
def test_v2v_agent_session_appends_history_across_turns(monkeypatch):
    """Two consecutive user turns share message history so the LLM has
    context. The second create() call must include the assistant reply
    from the first turn."""
    from gateway.platforms.voice_rtc import V2VAgentSession
    ...
    class _FakeChatCompletions:
        async def create(self, **kwargs):
            create_calls.append(kwargs)
            return _FakeStream(replies[len(create_calls) - 1])
    ...
    monkeypatch.setattr("gateway.platforms.voice_rtc.AsyncOpenAI", _FakeAsyncOpenAI)
    ...
    async def _go():
        for q in ("first", "second"):
            gen = await sess.submit_user_turn(q)
            async for _ in gen:
                pass
        await sess.close()
    ...
    # Second call's messages: system, user1, assistant1 ("AB"), user2.
    msgs2 = create_calls[1]["messages"]
    roles = [m["role"] for m in msgs2]
    assert roles == ["system", "user", "assistant", "user"]
    assert msgs2[2]["content"] == "AB"
    assert msgs2[3]["content"] == "second"
